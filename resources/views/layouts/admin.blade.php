...
<body class="bg-gray-100 font-family-karla flex">
    ...
    <aside class="relative bg-sidebar h-screen w-64 hidden sm:block shadow-xl">
        <div class="p-6">
            <a href="{{route ('admin.products')}}" class="text-white text-3xl font-semibold uppercase hover:text-gray-300">Admin</a>
        </div>
        <nav class="text-white text-base font-semibold pt-3">
            <a href="{{ route('admin.products') }}"
                class="flex items-center text-white opacity-75 hover:opacity-100 py-3 pl-6 nav-item">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor"
                    class="w-5 h-5 pl-0 mr-2.5">
                    <path
            ...
            <a href="/orders" class="flex items-center text-white opacity-75 hover:opacity-100 py-3 pl-6 nav-item">
    ...
    <div class="w-full flex flex-col h-screen overflow-y-hidden">
        ...
        <header class="w-full items-center bg-white py-2 px-6 hidden sm:flex">
            <div class="w-1/2 flex items-center">
                {{-- <img src="https://i.postimg.cc/595njGP5/brand.png" class="w-12 h-12" alt=""> --}}
            ...
            <div x-data="{ isOpen: false }" class="relative w-1/2 flex justify-end">
                <button @click="isOpen = !isOpen"
                    ...
                    <img src="https://i.postimg.cc/cJ9xgqC4/Whats-App-Image-2022-12-28-at-09-27-03-removebg-preview.png">
                </button>
                <button x-show="isOpen" @click="isOpen = false"
                    class="h-full w-full fixed inset-0 cursor-default"></button>
                <div x-show="isOpen" class="absolute w-32 bg-white rounded-lg shadow-lg py-2 mt-16">
                    <a href="{{ route('home.index') }}" tabindex="3"
                        class="text-gray-700 flex justify-between px-4 py-2 text-sm text-center"
                        role="menuitem">User</a>
                    <form method="POST" action="{{ route('logout') }}">
                        @csrf
                        <a href="{{ route('logout') }}"
                            onclick="event.preventDefault(); this.closest('form').submit();" tabindex="3"
                            class="text-gray-700 flex justify-between px-4 py-2 text-sm text-center" role="menuitem">Log
                            Out</a>
                    </form>
                </div>
            </div>
            ...
            <nav :class="isOpen ? 'flex' : 'hidden'" class="flex flex-col pt-4">
                ...
                <a href="/orders"
                    ...
                    <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 24 24"
                        stroke-width="1.5" stroke="currentColor" class="w-5 h-5 mr-2.5">
                        <path stroke-linecap="round" stroke-linejoin="round"
                            d="M2.25 3h1.386c.51 0 .955.343 1.087.835l.383 1.437M7.5 14.25a3 3 0 00-3 3h15.75m-12.75-3h11.218c1.121-2.3 2.1-4.684 2.924-7.138a60.114 60.114 0 00-16.536-1.84M7.5 14.25L5.106 5.272M6 20.25a.75.75 0 11-1.5 0 .75.75 0 011.5 0zm12.75 0a.75.75 0 11-1.5 0 .75.75 0 011.5 0z" />
                    </svg>
                    Orders
